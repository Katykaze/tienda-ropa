...
//---------------------EJERCICIO 1 LOGIN y LOGOUT DEL USUARIO---------------------
//verificamos que el usario esta dado de alta en la base de datos a través del numero de usuario que introduce y contraseña
function getNumberUserIfExists($conn, $number_user, $password)
{
    try {
        $valid = true;
        $stmt = $conn->prepare("SELECT customerNumber FROM customers WHERE customerNumber =:user AND contactLastName= :password");
